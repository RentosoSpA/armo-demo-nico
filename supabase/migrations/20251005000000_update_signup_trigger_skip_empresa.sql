-- Actualizar trigger para soportar skip_empresa flag
-- Cuando skip_empresa es true, solo crea el perfil sin empresa ni agente
...
DECLARE
  v_empresa_id uuid;
  v_company_name text;
  v_skip_empresa boolean;
BEGIN
  -- Verificar si el usuario quiere omitir la creación de empresa
  v_skip_empresa := COALESCE((NEW.raw_user_meta_data ->> 'skip_empresa')::boolean, false);

  -- Si skip_empresa es true, solo crear el perfil básico
  IF v_skip_empresa THEN
  ...
  -- Flujo normal: obtener el nombre de la empresa del metadata
