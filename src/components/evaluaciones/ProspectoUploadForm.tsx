...
const ProspectoUploadForm: React.FC<ProspectoUploadFormProps> = ({ prospectoId, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    try {
      if (!prospectoId) {
        message.error('ID de prospecto no válido');
        return;
      }
      ...
      await sendProspectoUploadNotification(prospectoId, 'Prospecto');
    ...
    <Card style={{ maxWidth: 800, margin: '0 auto' }}>
      <Space direction="vertical" size="large" className="w-full">
        <div className="text-center">
          ...
          <Text type="secondary">Sube los documentos requeridos para tu evaluación</Text>
        </div>
