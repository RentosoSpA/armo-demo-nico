...
const EvaluacionModal: React.FC<EvaluacionModalProps> = ({
  ...
  const handleEvaluar = async (aprobado: boolean) => {
    try {
      ...
      const prospectoName = `${prospecto.primer_nombre || ''} ${prospecto.primer_apellido || ''}`.trim() || 'Prospecto';
  ...
  return (
    ...
    >
      <Space direction="vertical" size="large" className="w-full">
        ...
        <Card size="small">
          <Descriptions title="Información de la Oportunidad" column={2}>
            <Descriptions.Item label="Nombre">
              {prospecto.primer_nombre} {prospecto.segundo_nombre} {prospecto.primer_apellido}{' '}
              {prospecto.segundo_apellido}
            </Descriptions.Item>
            <Descriptions.Item label="Email">{prospecto.email || 'No especificado'}</Descriptions.Item>
            <Descriptions.Item label="Documento">{prospecto.documento || 'No especificado'}</Descriptions.Item>
            <Descriptions.Item label="Teléfono">
              {prospecto.phone_e164 || 'No especificado'}
            </Descriptions.Item>
            <Descriptions.Item label="Situación Laboral">
              {prospecto.situacion_laboral || 'No especificado'}
            </Descriptions.Item>
            <Descriptions.Item label="Ingresos Mensuales">
              {prospecto.ingresos_mensuales
                ? `$${prospecto.ingresos_mensuales.toLocaleString('es-CL', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`
                : 'No especificado'}
            </Descriptions.Item>
          </Descriptions>
        </Card>
